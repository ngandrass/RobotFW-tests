...
"""@package PyToAPI
This module handles parsing of information from RIOT periph_utimer_cli test.
"""
import logging
...
class PeriphUTimerIf(DutShell):
    """Interface to the a node with periph_utimer_cli firmware."""
    ...
    DEFAULT_TIMER_CLK = "TIM_CLK_DEFAULT"
...
def main():
    """Test for PeriphUTimerIf."""

    logging.getLogger().setLevel(logging.DEBUG)
    try:
        putimer = PeriphUTimerIf()
        cmds = putimer.get_command_list()
        logging.debug("======================================================")
        for cmd in cmds:
            cmd()
            logging.debug("--------------------------------------------------")
